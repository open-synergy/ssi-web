...
odoo.define('ssi_web_widget_copy_content.CopyContentWidgets', function (require) {
    ...
    function copyTextToClipboard(text) {
        ...
        try {
            ...
            if (window.odoo && window.odoo.bus) {
                ...
                console.warn('Odoo bus not found, cannot display notification for error.');
    ...
    var CopyButtonMixin = {
        _addCopyButton: function (valueToCopy) {
            ...
            if (valueToCopy === null || valueToCopy === undefined || (typeof valueToCopy === 'string' && valueToCopy.trim() === '')) {
                return; 
            }
            ...
            var $button = $('<button/>', {
                ...
                type: 'button',
                title: 'Copy ' + (this.field.string || this.field.name) + ' to clipboard',
                'data-tooltip': 'Copy ' + (this.field.string || this.field.name) + ' to clipboard',
                'aria-label': 'Copy ' + (this.field.string || this.field.name) + ' to clipboard',
            });
            ...
            var $targetElement = this.$el.find('input.o_input, textarea.o_input, select.o_input, input.o_input_base, textarea.o_input_base, .o_field_many2one_selection, a.o_form_uri').last(); 
    ...
    var CopyBasicWidget = AbstractField.extend(CopyButtonMixin, {
        ...
        _render: function () {
            ...
            var text = value !== undefined && value !== null ? this._formatValue(value) : ''; 
    ...
    var CopyMany2oneWidget = FieldMany2One.extend(CopyButtonMixin, { 
        _render: function () {
            ...
            if (this.mode === 'readonly') {
                ...
                } else if (this.recordData[this.field.name] && this.recordData[this.field.name].data) {
                    valueToCopy = this.recordData[this.field.name].data.display_name || "";
                }
